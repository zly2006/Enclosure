...
public class EnclosureScreen extends HandledScreen<EnclosureScreenHandler> {
    ...
    protected void init() {
        ...
        permissionTargetListWidget = addChild(new PermissionTargetListWidget(client, area, handler.fullName, this, width, height, 60, height));
        globalWidget = addChild(new ButtonWidget(5, 35, 100, 20, new TranslatableText("enclosure.widget.global"), button -> {
            assert client != null;
            client.openScreen(new PermissionScreen(area, new UUID(0, 0), handler.fullName, this));
        }));
        playerWidget = addChild(new ButtonWidget(110, 35, 100, 20, new TranslatableText("enclosure.widget.player"), button -> {
            assert client != null;
            button.active = false;
            unlistedWidget.active = true;
            permissionTargetListWidget.showPlayers();
        }));
        unlistedWidget =addChild(new ButtonWidget(215, 35, 100, 20, new TranslatableText("enclosure.widget.unspecified_player"), button -> {
            assert client != null;
            button.active = false;
            unlistedWidget.active = true;
            permissionTargetListWidget.showPlayers();
        }));
        aboutWidget =addChild(new ButtonWidget(320, 35, 50, 20, new TranslatableText("enclosure.widget.about"), button -> {
            assert client != null;
            client.openScreen(new AboutScreen(this));
        }));
        ...
        textWidgets.add(new ClickableTextWidget(client, this, ((MutableText) MutableText.EMPTY)
